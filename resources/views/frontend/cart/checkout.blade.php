...
@section('content')
      ...
      <div class="my-md-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-3 flex-nowrap flex-xl-wrap overflow-auto overflow-xl-visble">
            ...
            <li class="breadcrumb-item flex-shrink-0 flex-xl-shrink-1 active" aria-current="page">Ödəmə səhifəsi</li>
          </ol>
        </nav>
      </div>
      <!-- End breadcrumb -->
  ...
  <checkout payment_url="{{ route('cart.store') }}"
            single_user_url="{{ auth()->check() ? route('user.singleUser',auth()->id()) : null }}"
            user_id="{{ auth()->check() ? auth()->id() : null }}"
            user_signin_url="{{ route('user.giris') }}"
            is_logged="{{ auth()->check() ? 1 : 2 }}"
            lost_password_url="{{ route('user.password.request') }}"></checkout>
@endsection
@section('scripts')
