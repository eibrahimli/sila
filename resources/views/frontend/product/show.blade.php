...
@section('title', ucfirst($product->title).' başlıqlı məhsul')

@section('content')
  <!-- breadcrumb -->
  @include('frontend.inc.breadcrumb',['product' => $product])
  <!-- End breadcrumb -->

  <div class="container">
    <product-show add_to_cart="{{ route('product.addToCart',$product->id) }}" get_product="{{ route('product.getProduct', $product->id) }}"></product-show>
    <!-- Brand Carousel -->
    <!-- Related products -->
    @if($related_products)
    <div class="mb-6">
          ...
          <div class="product-item__outer h-100">
            <div class="product-item__inner px-xl-4 p-3">
              <div class="product-item__body pb-xl-2">
                <div class="mb-2">
                  <a href="{{ $rproduct->category->url }}" class="font-size-12 text-gray-5">{{ $rproduct->category->name }}</a></div>
                <h5 class="mb-1 product-item__title">
                  <a href="{{ $rproduct->url }}" class="text-blue font-weight-bold">{{ $rproduct->title }}</a>
                </h5>
                <div class="mb-2">
                  <a href="{{ $rproduct->url }}" class="d-block text-center">
                    <img class="img-fluid" src="{{ asset('storage/'.$rproduct->photo) }}" alt="Image Description">
                  </a>
                </div>
                <div class="flex-center-between mb-1">
                  <div class="prodcut-price">
                    <div class="text-gray-100">₼{{ $rproduct->price }}</div>
                  </div>
                </div>
              </div>
            </div>
