        ...
        <div v-show="!loading">
            <div class="row m-b-30">
                ...
                <div class="form-group col-12 col-sm-12">
                    ...
                    <div v-if="!$v.product.desc.required && $v.product.desc.$dirty" style="display: block"
                         class="invalid-feedback">
                        Açar sözlər boş ola bilməz...
                    </div>
                    <div v-if="!$v.product.desc.minLength && $v.product.desc.$dirty" style="display: block"
                         class="invalid-feedback">
                ...
                <div class="form-group col-12">
                    ...
                    <input type="number" v-model.number="product.stock" class="form-control" id="stock"
                           aria-describedby="stockHelp">
                    <small id="stockHelp" class="form-text text-warning">Ümümi məhsulun stokda neçə dənə olduğu qeyd
                        edilməlidi...
                    </small>
                    <div v-if="!$v.product.stock.required && $v.product.stock.$dirty" style="display: block"
                         class="invalid-feedback">
                        Məhsul stok daxil edilməlidi...
                    </div>
                    <div v-if="!$v.product.stock.integer && $v.product.stock.$dirty" style="display: block"
                         class="invalid-feedback">
            ...
            <ul class="nav nav-pills justify-content-between wizard m-b-30">
                <li class="nav-item next">
                    <a class="nav-link btn display-4 btn-success" @click.prevent="submit">Yenilə</a>
                </li>
            </ul>
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="content">
                        ...
                        <div v-if="typeof validationErrors2 != 'string'">
                            <div v-for="err in validationErrors2" :key="err">{{ err }}</div>
                        </div>
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <div class="row gutters-sm">
                            <div v-for="(img,index) in photos" :key="index" class="col-md-4 col-sm-6">
                                <label class="image-box mb-4">
                                    <input type="checkbox" v-model="checkedNames" :value="img.id">
                                    <span class="image-box-content">
                    <img width="212" height="220" :src="base_url+`/storage/${img.photo}`" alt="">
                </span>
                                </label>
                            </div>
                            <button @click="deletePhotos($event)" :class="{'disabled' : checkedNames.length == 0}"
                                    class="btn btn-danger col-md-12 col-sm-12">Sil
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div v-if="hasColor" class="table-responsive-sm">
                        ...
                        <table class="table">
                            ...
                            <tr v-for="(productColor, index) in productColors" :key="index" class="mt-1">
                                <td>
                                    <select v-model="productColor.color_id" class=" mt-1"
                                ...
                                <td>
                                    <img :src="productColor.path"
                                         :alt="colors.find((color) => color.id == productColor.color_id).label"
                                         width="60" height="60"
                                    >
                                    <input type="file" @change="productColor.photo = $event.target.files[0]"
                                           class="mt-1">
                                </td>
                                <td>
                                    <input type="number" class="form-control mt-1"
                                           v-model.number.trim="productColor.quantity">
                                </td>
                                <td>
                                    <button
                                        class="btn btn-small btn-primary col-12 mb-1"
                                        @click="updateColorStock(productColor.id,productColor.color_id,productColor.photo,productColor.quantity,index)">
                                        Yenilə
                                    </button>
                                    <button @click.prevent="deleteColorStock(productColor.id,index)" class="btn btn-small btn-danger col-12 mb-1">Sil</button>
                                </td>
                            </tr>
                            </tbody>
                        </table>
                        ...
                        <table class="table">
                            ...
                            <tr class="mt-1">
                                <td>
                                    <select v-model="color_id" class=" mt-1" aria-placeholder="Rəng seçin...">
                                        <option value="0" selected disabled>Rəng seçin...</option>
                                        <option
                                            v-if="!productColors.find((productColor) => { return productColor.color_id == color.id})"
                                            v-for="color in colors" :value="color.id">{{ color.label }}
                                        </option>
                                    </select>
                                </td>
                                <td>
                            ...
                            <button class="ml-3 btn btn-dark btn-small color-white" @click.prevent="addStock">Əlavə et
                            </button>
                        </table>
                    ...
                    <div class="table-responsive-sm">
                        ...
                        <table class="table">
                            ...
                            <tr class="mt-1" v-for="(spec , index) in specs" :key="index">
                                <td>
                                    <div class="form-group">
                                        <input type="text" class="form-control" v-model.trim="spec.name"
                                               placeholder="Xüsusiyyət">
                                    </div>
                                </td>
                                <td>
                                    <div class="form-group">
                                        <input type="text" class="form-control" v-model.trim="spec.value"
                                               placeholder="Dəyər">
                                    </div>
                                </td>
                            </tr>
                            </tbody>
                            <button class="ml-3 btn btn-dark btn-small color-white" @click.prevent="addSpec">Əlavə et
                            </button>
                        </table>
            ...
            <ul class="nav nav-pills justify-content-between wizard m-b-30">
                <li class="nav-item next">
                    <a class="nav-link btn display-4 btn-success" @click.prevent="updateProduct">Yenilə</a>
                </li>
            </ul>
        </div>
...
export default {
    ...
    mounted() {
        ...
        axios.get(this.getproducturl).then(res => this.product = res.data.product)
    ...
    methods: {
        ...
        addStock: function () {
            ...
            axios.post(Laravel.base_url + `/seller/product/${this.id}/update/stock`, data)
                .then(res => {
                    axios.get(this.getcolorsrelatedtoproduct)
                        .then(res => {
        ...
        updateProduct: function () {
            ...
            data.set('_method', 'PATCH')
            axios.post(this.update_specs, data)
                .then(res => {
                    Swal.fire({
                        icon: 'success',
                        title: res.data.mes,
        ...
        deleteColorStock: function (id,index) {
            ...
            axios.post(this.deletesingleproductcolor, { "stock": id })
                .then(res => {
